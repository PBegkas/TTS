...
public class LineSpeech implements ActionListener{
	...
	*/
	...
	//constructor
	public LineSpeech(JTextArea textArea, Document curDocument, int chosenFunc,ArrayList<ActionListener> commandList,ArrayList<Boolean> replayBool) {
	...
	//test constructor
	public LineSpeech(int test,JTextArea textArea, Document curDocument, int chosenFunc,ArrayList<ActionListener> commandList,ArrayList<Boolean> replayBool) {
		this.test = test;
		this.textArea = textArea;
		this.curDocument = curDocument;
		this.chosenFunc = chosenFunc;
	...
	//replay constructor
	public LineSpeech(JTextArea textArea, Document curDocument, int chosenFunc,int caretPos, ArrayList<ActionListener> commandList) {
		this.textArea = textArea;
		this.curDocument = curDocument;
		this.chosenFunc = chosenFunc;
		this.commandList = commandList;
		...
		if(chosenFunc == 1) {
			...
			if(!isReplayed && test == 0) {
				if(replayBool.get(0) == true) {
					LineSpeech copy = new LineSpeech(textArea, curDocument, chosenFunc, caretPos,  commandList);
